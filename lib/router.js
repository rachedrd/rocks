...
Router.route('/reset-password::token', {name :"homeReset" , onBeforeAction: function(){
this.next();
...
Router.route('/events', {name :"homeEvents" /*, waitOn: function()
						            {
						            	return [IRLibLoader.load('/js/zwindows.js')]
						            }*/});
...
Router.route('/myprofile', {name :"homeProfile",  onBeforeAction: function(){
 ...
 if(! Meteor.userId()) 
 		...
 		else //if(Meteor.user() !== undefined)
 		{
            ...
            {
    		if(Meteor.user().profile.Status === "inactive")
 			{
 				Router.go('/');
 			}
